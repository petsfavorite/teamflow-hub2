  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={Wrench} label="Open Requests" value={openMaintenance.length} color="bg-amber-600" to={createPageUrl('Maintenance')} />
        <StatCard icon={ExternalLink} label="App Links" value={links.length} color="bg-purple-600" to={createPageUrl('ExternalLinks')} />
      </div>
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-slate-900">Recent Maintenance Requests</h2>
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Link to={createPageUrl('SOPAssistant')} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-indigo-50 hover:bg-indigo-100 transition-colors">
                ...
                <span className="text-sm font-medium text-indigo-700">Ask SOP AI</span>
              </Link>
              <Link to={createPageUrl('Checklists')} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-emerald-50 hover:bg-emerald-100 transition-colors">
                <CheckSquare className="w-6 h-6 text-emerald-600" />
                <span className="text-sm font-medium text-emerald-700">My Checklists</span>
              </Link>
              <Link to={createPageUrl('Maintenance') + '?new=true'} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-amber-50 hover:bg-amber-100 transition-colors">
                ...
                <span className="text-sm font-medium text-amber-700">New Request</span>
              </Link>
              <Link to={createPageUrl('SOPs')} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-purple-50 hover:bg-purple-100 transition-colors">
                <BookOpen className="w-6 h-6 text-purple-600" />
                <span className="text-sm font-medium text-purple-700">Browse SOPs</span>
              </Link>
            </div>
          </CardContent>
        </Card>
