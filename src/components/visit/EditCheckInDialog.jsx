...
export default function EditCheckInDialog({ pet, visit, open, onClose, onSave }) {
    ...
    const handleConfirm = async (newData) => {
        ...
        // Exclude old uncompleted Play Session tasks — they are regenerated by the
        // form, so keeping them would create duplicates. Completed ones are kept
        // so the history of finished sessions is preserved.
        const existingNonTemplateTasks = (visit.scheduled_tasks || []).filter(t =>
            t.type !== 'Billing' && !(t.type === 'Play Session' && !t.completed) && (!t.is_template || t.completed)
        );
